...
public class WorkerWrapper<T, V> {
    ...
    public void work(ThreadPoolExecutor poolExecutor, long timeout, Map<String, WorkerWrapper> forParamUseWrappers) {
        work(poolExecutor, null, timeout, forParamUseWrappers);
    }
     ...
     */
    private void work(ThreadPoolExecutor poolExecutor, WorkerWrapper fromWrapper, long timeout, Map<String, WorkerWrapper> forParamUseWrappers) {
        ...
        //总的已经超时了，就快速失败，进行下一个
        if (timeout <= 0) {
            System.out.println(">>>>>>>> 已超时，快速失败 <<<<<<<<");
            fastFail(INIT, null);
            beginNext(poolExecutor, now, timeout);
            return;
        }
        //todo 稍后验证一下
        //如果自己已经执行过了。
        ...
        if (getState() == FINISH || getState() == ERROR) {
            System.out.println(">>>>>>>> 状态等于完成或失败 : " + getState() + " <<<<<<<<");
            beginNext(poolExecutor, now, timeout);
            return;
        }
        ...
        if (needCheckNextWrapperResult) {
            ...
            if (!checkNextWrapperResult()) {
                System.out.println(">>>>>>>> 当前调用链存在已完成任务 <<<<<<<<");
        ...
        if (dependWrappers.size() == 1) {
            ...
            //有多个依赖时
            doDependsJobs(poolExecutor, dependWrappers, fromWrapper, now, timeout);
        }
     ...
     * 进行下一个任务
     */
    private void beginNext(ThreadPoolExecutor poolExecutor, long now, long timeout) {
        ...
        if (nextWrappers.size() == 1) {
            nextWrappers.get(0).work(poolExecutor, WorkerWrapper.this, timeout - costTime, forParamUseWrappers);
            return;
        }
        ...
        for (int i = 0; i < nextWrappers.size(); i++) {
            ...
            futures[i] = CompletableFuture.runAsync(() -> nextWrappers.get(finalI)
                    .work(poolExecutor, WorkerWrapper.this, timeout - costTime, forParamUseWrappers), poolExecutor);
        }
        try {
    ...
    private synchronized void doDependsJobs(ThreadPoolExecutor poolExecutor, List<DependWrapper> dependWrappers, WorkerWrapper fromWrapper, long now, long remainTime) {
        ...
        if (mustWrapper.size() == 0) {
            ...
            System.out.println("=== 非必要条件 执行自己 ===");
            beginNext(poolExecutor, now, remainTime);
            return;
        }

        //如果存在需要必须完成的，且 fromWrapper 不是必须的，就什么也不干
        if (!nowDependIsMust) {
            System.out.println("======== nowDependIsMust ");
            return;
        }
